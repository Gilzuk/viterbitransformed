...
if __name__ == '__main__':
  ...
  for mc in range(100):
    ...
    for snr, val_block_length,pilot_length in parameters:
      ...
      perf_tracker.save_to_csv(f"model_performance_snr_{snr}.csv")
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    perf_tracker.save_to_csv(f"model_performance_final_mc_{mc}_{timestamp}.csv")
