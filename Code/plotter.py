...
def plot_ser_by_snr(all_curves: List[Tuple[np.ndarray, np.ndarray, str]], snr_values: List[float]):
    ...
    for method_name in names:
        ...
        plt.plot(snr_values, mean_sers, label=METHOD_NAMES[key],
                 color=COLORS_DICT[key], marker=MARKERS_DICT[key],
                 linestyle=LINESTYLES_DICT[key], linewidth=1.5)

    plt.xticks(snr_values, snr_values,fontsize=14)
    plt.yticks(fontsize=12)
    plt.xlabel('SNR [dB]',fontdict={'size': 14})
    plt.ylabel('Coded BER',fontdict={'size': 14})
...
def save_pkl(pkls_path: str, array: np.ndarray):
    ...
    str_time = date_time.strftime("%I_%M_%S")
    date_cal = datetime.datetime.fromtimestamp(timestamp)
    str_date = date_cal.strftime("%d_%B_%Y")
    new_path = pkls_path.replace('.pkl', "_"+str_date+"_"+str_time+".pkl")
    output = open(new_path, 'wb')
    pkl.dump(array, output)
    output.close()
    
def _save_pkl(pkls_path: str, array: np.ndarray):
