...
def execute_model(model, detector_method, self_supervised, all_curves, current_params, run_over,num_of_rep,hyperparams_dict):
    if model == 'ClassicViterbi':
        execute_and_plot(model, 'Statistical', False, all_curves, current_params, run_over,num_of_rep,hyperparams_dict)  # Classic Viterbi Alg with Perfect-CSI
    else:
        execute_and_plot(model, detector_method, self_supervised, all_curves, current_params, run_over,num_of_rep,hyperparams_dict)
...
if __name__ == '__main__':
    for mc in range(20):
        ...
        # models_list = ['Transformer', 'Sionna','ViterbiNet']
        ...
        for snr in range(snr_start, snr_end+1):
            ...
            HYPERPARAMS_DICT['pilots_num'] = 25*num_of_reps[snr-snr_start]
            ...
            for model in models_list:
                ...
                process = multiprocessing.Process(target=execute_model, args=(model, detector_method, self_supervised, all_curves, current_params, run_over,num_of_reps[snr-snr_start], HYPERPARAMS_DICT))
