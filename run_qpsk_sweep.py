...
FRAMES = 64
RECEIVERS = ['classic_csi', 'classic_ls', 'tied@5', 'sym_affine@10', 'sym_mlp@10', 'affine@10', 'mlp@10']
ITERS = {'tied@5': 5, 'sym_affine@10': 10, 'sym_mlp@10': 10, 'affine@10': 10, 'mlp@10': 10}

cfg = Config(dd_lr=0.01)
new = not os.path.exists(OUT)
with open(OUT, 'a', newline='') as f:
    w = csv.writer(f, lineterminator='\n')
    if new:
        w.writerow(['receiver', 'params', 'es_n0_db', 'ser', 'symbol_errors', 'symbols', 'M', 'L', 'rho',
                    'pilot_words', 'pilot_iters', 'online_iters', 'adapt', 'frames', 'pilot_lr', 'dd_lr'])
    for snr in SNRS:
        t0 = time.time()
        ...
        for r, (e, n) in stats.items():
            w.writerow([r, npar.get(r, {'classic_csi': 0, 'classic_ls': 2 * cfg.L + 1}[r] if r.startswith('classic') else ''),
                        ...
                        ITERS.get(r, ''), cfg.adapt, FRAMES, cfg.online_lr, cfg.dd_lr])
        f.flush()
